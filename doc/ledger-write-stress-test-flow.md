...
- Confirm that no previous test has an active request.
- Use a new `run_id` so database rows can be isolated from other runs.

This test calls `PostService/postTransaction` directly through gRPC. That path
invokes `PostLedgerProcessor` and persists `ledger_transactions` and
`ledger_entries`; it does not create an outbox row. Reply outboxes belong to the
Kafka command-listener path and are outside this test's acceptance criteria.
...
5. Stop before prolonged timeouts or backlog growth can destabilize the test
   environment.
6. Set the arrival rate to zero and allow in-flight requests to finish before
   reconciliation.
...
k6 stopping new iterations does not prove that all in-flight RPCs completed.
...
### Entry Payload Integrity

For each unique committed transaction, compare the persisted entry account,
asset, direction, and amount with the deterministic input manifest. Calculate
expectations from unique logical transactions rather than raw request attempts,
because duplicates must not create additional effects. Do not assume a timed-out
request failed; use its persisted final state.

Do not verify account balances in this ledger-service test. Ledger-service is a
passive journal and does not maintain account balance state. Balance and
reservation reconciliation belong to the later wallet-service stress test.
...
AND persisted entries matched the generated journal payload
AND duplicates produced no additional durable effects
AND active requests returned to baseline
```
