...
public class WriteOpsImpl<T> implements SimpleWriteOps<T> {
...
//        Result<CacheValueInfo<T>> refCacheResult = simpleRedisSupport.get(cacheKey, cacheDescriptor.getClazz());
...
//                Result<Boolean> deleteResult = simpleRedisSupport.delete(cacheKey);
...
//    }
    ...
    public Result<Void> set(String id, T value, TtlStrategy ttl) {
        String cacheKey = cacheDescriptor.buildCacheKey(id);
        Result<Void> setResult = simpleRedisSupport.set(cacheKey, value, ttl);
